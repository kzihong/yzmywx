...
<script type="text/javascript">
	...
	var submitNow = function($dialog, $grid, $pjq) {
		...
		if ($(':input[name="id"]').val().length > 0) {
			url = sy.contextPath + '/rest/user/update';
		} else {
			url = sy.contextPath + '/rest/user/save';
		}
	...
	$(function() {
		...
		if (userid && userid.length > 0) {
			...
			var dataurl = sy.contextPath + '/rest/user/getById/' + userid;
			$.get(dataurl, function(result) {
				if (result.id != undefined) {
					var deparmentid = "";
					if(result.department)deparmentid = result.department.id;
					var jobid = "";
					if(result.department)jobid = result.job.id;
					$('form').form('load', {
						'id' : result.id,
						'name' : result.name,
		...
		<fieldset>
			...
			<table class="table" style="width: 100%;">
				...
				<tr>
					...
					<th>性别</th>
					<td><select class="easyui-combobox" name="sex" data-options="panelHeight:'auto',editable:false,required:true" style="width: 155px;">
