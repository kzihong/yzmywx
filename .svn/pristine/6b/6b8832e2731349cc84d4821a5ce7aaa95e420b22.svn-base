...
public class EditorRest {
	...
	public String uploadPic(SearchData searchData){
		...
		try {
			...
			String c[] = b[1].split(";");
			String fileType = c[0];
			String serverPathString = OrderUtil.getUUID()+"."+fileType;
			String imgFilePath = ConfigUtil.get("newsPhotoPath")+"/"+d[1]+"/";
			System.out.println(imgFilePath);
			System.out.println(a[1]);
			if(Base64Util.string2Image(a[1], imgFilePath,serverPathString)){
				returnData.setErrmsg("/rest/editor/getPIC"+"/"+d[1]+"/"+serverPathString);
	...
	public String downloadPic(@PathParam("mediaid") String mediaid){
		...
		try {
			...
			File file = WeixinUtil.downloadMedia(ConfigUtil.get("newsPhotoPath") +"/image/",fileName, getAT(), mediaid);
			if(file != null){
